...
impl PPMImg {
    ...
    pub fn write_bin_to_buf(&self, writer: &mut impl Write) -> io::Result<()> {
    ...
    pub fn write_binary(&self, filepath: &str) -> io::Result<()> {
        self.write_bin_to_buf(&mut create_file(filepath))
    }
    pub fn write_ascii(&self, filepath: &str) -> io::Result<()> {
        let mut file = BufWriter::new(create_file(filepath));
        writeln!(file, "P3")?;
